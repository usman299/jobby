...
class JobberController extends Controller
{
    ...
    public function jobberReviews(){
        ...
        if (!$reviews->isempty()){
            ...
            $total = $reviews->count();
            $totalReviews = round($totalReview / $total);
            $fiveStar = (Reviews::where('star', '=', 5)->where('reciver_id','=',$user->id)->count() / $total) * 100;
            $fourStar = (Reviews::where('star', '=', 4)->where('reciver_id','=',$user->id)->count() / $total) * 100;
            $threeStar = (Reviews::where('star', '=', 3)->where('reciver_id','=',$user->id)->count() / $total) * 100;
            $twoStar = (Reviews::where('star', '=', 2)->where('reciver_id','=',$user->id)->count() / $total) * 100;
            $oneStar = (Reviews::where('star', '=', 1)->where('reciver_id','=',$user->id)->count() / $total) * 100;
        }
        else{
            $reviews =null;
            $totalReviews=0;
