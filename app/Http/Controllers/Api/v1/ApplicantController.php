...
{
    public function jobRequestSubmit(Request $request, $id, $check = null)
    {
        $user = Auth::user();
        if ($check == 1) {
        ...
        try {
            ...
            $jobrequest->category_id = $cat->category->id;

            if ($check == 1) {
                $jobrequest->subcategory_id = $cat->subcategory->id;
