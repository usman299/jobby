...
@section('content')
    ...
    <section class="emPage__detailsBlog">
        ...
        <div class=" margin-t-20 padding-20 d-flex emBlock__border">
            ...
            @if($contract->status==1)
                <a href="{{route('applicant.contract.status', ['id' => $contract->id,'status'=>2])}}"
                   class="btn bg-green rounded-10 btn__default ml-3">
        ...
        @if($contract->jobber_id_applicant==null)
            @if($contract->status==3)
                <div class="margin-b-20  padding-20 d-flex emBlock__border">
                    <a href="" class="btn bg-secondary rounded-0 btn__default full-width" data-toggle="modal"
                       data-target="#acceptpropsal">
